const handlePersonalInfo = (event) => {
    ...
    fetch("http://127.0.0.1:8000/accounts/personal-info/", {
        ...
        headers: {
            ...
            "X-CSRFToken": getCsrfToken(),  // CSRF token included if necessary
        },
        body: JSON.stringify(info),  // Send the form data as the body
        credentials: "same-origin",  // Ensure cookies (like sessionid) are sent with the request
    })
        .then(async (res) => {
            if (res.ok) {
...
// Function to get the CSRF token from the cookies (if necessary)
